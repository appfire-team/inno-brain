...
INTENT_GROUPS: list[dict[str, Any]] = [
    {
        ...
        "intents": {
            ...
            "outside_the_moat": "Outside the moat — new wedges Appfire hasn't tried, with GTM motion",
...
def intent_instruction(intent: str | None, ws: Any = None) -> str:
    ...
    mapping = {
        ...
        "outside_the_moat": (
            "Appfire has built its position via the Atlassian Marketplace and bundling "
            "around known product lines (CMJ, JMWE, Spartez, Comala, etc.). The user "
            "wants NEW opportunities Appfire has NOT done yet — AND a credible plan to "
            "execute them OUTSIDE the existing Marketplace channel. The corpus is your "
            "map of what's already known; your job is the territory outside it.\n\n"
            "Two jobs at once:\n"
            "  1. Surface 3-5 wedges the corpus does NOT yet evidence as known Appfire "
            "     patterns. Explicitly avoid recommendations of the shape 'bundle this "
            "     into CMJ' / 'ship as a JMWE add-on' / 'reuse Comala distribution'. If "
            "     your idea reduces to 'do what Appfire already does, plus a feature', "
            "     it's not a new wedge — discard it and find a real one. Treat strong "
            "     corpus evidence for a known Appfire pattern as a RISK of recommending "
            "     it, not a reason.\n"
            "  2. For EACH wedge, name HOW Appfire would actually ship it without "
            "     leaning on the Marketplace. 'TBD' is not an answer. Stake a specific "
            "     motion from: direct enterprise sales, product-led growth (free tier "
            "     + paid expansion), vertical resellers, channel partners outside the "
            "     Atlassian ecosystem, content / community / open-source funnel, "
            "     regulatory or industry certification as channel, named partnerships, "
            "     small anchor acquisition + reskin.\n\n"
            "For EACH wedge output exactly this structure:\n"
            "  • THE WEDGE: one sentence on what we'd build and for whom. Be specific "
            "    about the buyer — firmographic (industry · size · stage) + role + the "
            "    trigger event that makes them shop. 'Eng leaders at Series B fintechs "
            "    failing their first SOC 2' beats 'mid-market companies'.\n"
            "  • WHY THIS IS NEW FOR APPFIRE: one sentence on what makes this different "
            "    from anything Appfire ships today. If it's adjacent to an existing "
            "    Appfire product, name that product explicitly and the specific gap "
            "    this fills that the existing product cannot. If it's fully greenfield, "
            "    say so.\n"
            "  • GO-TO-MARKET MOTION: the specific channel. Name the first 3 customer-"
            "    acquisition mechanisms in order. Examples: 'Direct enterprise sales — "
            "    AE prospects 200 Series B fintechs via LinkedIn + Signal + warm intros "
            "    from existing CFO advisors'; 'PLG — free tier on Vercel/Render with "
            "    Stripe self-serve at $99/seat, then SE-led expansion to 50+ seat orgs'; "
            "    'Channel — sign 5 boutique compliance consultancies as resellers in "
            "    Q1 with 30% margin'. Specific names, not categories.\n"
            "  • UNFAIR ADVANTAGE WE'D BORROW: what Appfire DOES have today that "
            "    transfers to this new motion (capital, brand among Jira admins, "
            "    M&A team, partnership network with Atlassian / AWS / etc., engineering "
            "    bench, security/compliance posture, public-company-readiness). Don't "
            "    claim we have things we don't.\n"
            "  • CHEAPEST PROOF (90 days, < $50K): the one experiment that flips the "
            "    verdict from 'maybe' to 'yes, hire the GM'. Name the specific metric "
            "    that has to move (e.g. '5 paid pilots at $50K ACV', '$100K committed "
            "    pipeline from outbound', 'reseller channel produces 10 qualified "
            "    intros'). 'Run a workshop' is not a proof.\n"
            "  • HONEST ODDS: tail bet (< 10%) / long shot (10-30%) / real chance "
            "    (> 30%). State the dominant reason it'd fail.\n\n"
            "Rules for output:\n"
            "- Be ruthless about NEW. If the rubric's Sherlocking test or capital rule "
            "  kills it, name that AND describe what would have to be true to flip the "
            "  verdict (a partner takes the channel risk, a vertical's pain crosses a "
            "  pricing threshold, regulation forces buyer behavior). Don't silently "
            "  drop wedges that don't pass the rubric — name them and note the lever.\n"
            "- Prefer named, specific buyer + named, specific channel. Categories "
            "  ('SMB', 'PLG') without names are unactionable.\n"
            "- Cross-community is a feature, not a bug. If your wedges all reference "
            "  the same corpus community, you haven't pushed hard enough — go again "
            "  and surface at least one wedge from a community that received fewer "
            "  than 10% of the corpus's nodes.\n"
            "- End with: 'THE WEDGE I'D STAKE THE MOST ON: <name> — <one sentence on "
            "  why the GTM motion threads a needle competitors and rubric biases miss>. "
            "  Owner: <which Appfire leader / role should own it — CRO, CPO, VP "
            "  Corp Dev, VP New Markets, etc.>.'"
        ),
